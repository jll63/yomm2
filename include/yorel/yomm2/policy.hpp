...
template <class Class, class Policy> struct virtual_ptr;

template <typename T> struct virtual_;

template <class Policy, typename Key, typename Signature> struct method;

template <class... Classes> struct class_declaration;
...
using error_type = std::variant<resolution_error, unknown_class_error,
                                hash_search_error, method_table_error>;
...
template <class Key> struct yOMM2_API_gcc yOMM2_API_msc method_tables {
...
template <class Key> catalog basic_domain<Key>::catalog;

template <class Key>
std::vector<std::uintptr_t> basic_domain<Key>::dispatch_data;

struct rtti {
  static type_id type_index(type_id type) { return type; }
...
struct std_rtti : virtual rtti {
  ...
  template <typename D, typename B> static D dynamic_cast_(B &&obj) {
...
template <typename T> constexpr bool implemented = !std::is_same_v<T, void>;

template <typename Policy, class Facet> struct rebind_facet {
  using type = Facet;
};

template <typename NewPolicy, typename OldPolicy,
...
struct basic_policy : virtual abstract_policy,
  ...
  using use_facet = boost::mp11::mp_first<boost::mp11::mp_filter_q<
      boost::mp11::mp_bind_front_q<boost::mp11::mp_quote_trait<std::is_base_of>,
                                   Facet>,
      facets>>;

  template <class NewPolicy>
  using rebind =
      basic_policy<NewPolicy,
                   typename rebind_facet<NewPolicy, Facets>::type...>;
  ...
  using remove = boost::mp11::mp_apply<
      ...
      boost::mp11::mp_push_front<
          boost::mp11::mp_remove_if_q<
              facets, boost::mp11::mp_bind_front_q<
                          boost::mp11::mp_quote_trait<std::is_base_of>, Base>>,
          Policy>>;
};
...
struct yOMM2_API_gcc vptr_vector : virtual external_vptr {
  ...
  static void register_vptrs(ForwardIterator first, ForwardIterator last) {
    ...
    if constexpr (has_facet<Policy, type_hash>) {
      size = Policy::hash_initialize(pair_first_iterator(first),
                                     pair_first_iterator(last));
    } else {
...
template <class Policy> struct yOMM2_API_gcc vptr_map : virtual external_vptr {
  ...
  template <class Class> static auto dynamic_vptr(const Class &arg) {
...
template <typename ForwardIterator>
size_t
fast_perfect_hash<Policy>::hash_initialize(ForwardIterator first,
  ...
  for (size_t pass = 0; pass < 4; ++pass, ++M) {
    ...
    if constexpr (has_output) {
      Policy::update_stream << "  trying with M = " << M << ", " << hash_size
                            << " buckets\n";
    }
...
template <class Policy> type_id fast_perfect_hash<Policy>::mult;

template <class Policy> std::size_t fast_perfect_hash<Policy>::shift;
...
struct yOMM2_API_gcc checked_perfect_hash : virtual fast_perfect_hash<Policy>,
  ...
  template <typename ForwardIterator>
  static size_t hash_initialize(ForwardIterator first, ForwardIterator last) {
    return fast_perfect_hash<Policy>::hash_initialize(first, last, control);
  }
};
  ...
  static void default_error_handler(const error_type &error_v) {
    ...
    if constexpr (Policy::template has_facet<error_output>) {
      if (auto error = std::get_if<resolution_error>(&error_v)) {
        const char *explanation[] = {"no applicable definition",
                                     "ambiguous call"};
        Policy::error_stream
        ...
        for (auto ti : type_range{error->tis, error->tis + error->arity}) {
        ...
        Policy::error_stream << ")\n";
      } else if (auto error = std::get_if<unknown_class_error>(&error_v)) {
...
struct yOMM2_API_gcc backward_compatible_error_handler
  ...
  static void default_call_error_handler(const method_call_error &error,
                                         size_t arity, type_id ti_ptrs[]) {
...
struct yOMM2_API_gcc basic_static_policy
    : basic_policy<Policy, vptr_vector<Policy>, std_rtti,
                   vectored_error_handler<Policy>, Facets...> {};

template <class Policy, class... Facets>
struct yOMM2_API_gcc basic_debug_static
    : basic_static_policy<Policy, checked_perfect_hash<Policy>,
                          basic_error_output<Policy>,
...
struct yOMM2_API_gcc debug_shared
    : basic_policy<debug_shared, vptr_vector<debug_shared>, std_rtti,
                   checked_perfect_hash<debug_shared>,
                   basic_error_output<debug_shared>,
                   basic_update_output<debug_shared>,
                   backward_compatible_error_handler<debug_shared>> {};
